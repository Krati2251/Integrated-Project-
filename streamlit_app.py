...
def _quote_mailbox(mailbox: str) -> str:
    """Return a mailbox name quoted the way Gmail IMAP expects."""
...
def fetch_emails_from_gmail(include_read: bool = True, max_emails: int = 5) -> dict:
    ...
    try:
        lookback_days = int(_secret("EMAIL_LOOKBACK_DAYS", "30"))
        since_date = (datetime.now() - timedelta(days=lookback_days)).strftime("%d-%b-%Y")
        include_sent = _secret("EMAIL_INCLUDE_SENT", "false").lower() == "true"
        fetch_all = _secret("EMAIL_FETCH_ALL", "false").lower() == "true"

        if include_read or fetch_all:
            search_plan = [
            ...
            search_plan = [("INBOX", f'(UNSEEN SINCE "{since_date}")')]
            if include_sent:
                search_plan.extend([
                    ("[Gmail]/Sent Mail", f'(SINCE "{since_date}")'),
                    ("[Gmail]/All Mail", f'(SINCE "{since_date}")'),
        ...
        if not email_ids:
            db.close()
            return {"fetched": 0, "errors": 0, "error_details": [],
                    "tickets": [], "message": "No emails found."}
...
def calculate_dashboard_metrics(db) -> dict:
    ...
    sla_breaches = [
        ...
        if t.priority == TicketPriority.HIGH and _to_utc_naive(t.created_at) and _to_utc_naive(t.created_at) < sla_threshold
    ]
    ...
    for cat in [TicketCategory.FRAUD, TicketCategory.PAYMENT_ISSUE, TicketCategory.GENERAL]:
        ...
        cat_avg_h = 0.0
        if cat_closed:
            deltas = [(now - _to_utc_naive(t.created_at)).total_seconds() / 3600 for t in cat_closed if _to_utc_naive(t.created_at)]
...
def _api_fetch_emails(include_read: bool = True, max_emails: int = 5):
    try:
...
# ═══════════════════════════════════════════════════════
#  AUTO-REFRESH: every 5 min (configurable) to keep alive
# ═══════════════════════════════════════════════════════
try:
    _AUTO_REFRESH_MS = int(_secret("UI_REFRESH_INTERVAL_MS", "300000"))
except Exception:
    _AUTO_REFRESH_MS = 300_000
st_autorefresh(interval=_AUTO_REFRESH_MS, limit=None, key="auto_refresh_5min")

# ── Auto-fetch emails on each refresh cycle ──
_now = _time.time()
try:
    _FETCH_INTERVAL = int(_secret("EMAIL_POLL_INTERVAL", "300"))
except Exception:
    _FETCH_INTERVAL = 300
_ENABLE_EMAIL_POLLING = _secret("ENABLE_EMAIL_POLLING", "true").lower() == "true"
_INCLUDE_READ = _secret("EMAIL_INCLUDE_READ", "true").lower() == "true"
_FETCH_ALL = _secret("EMAIL_FETCH_ALL", "false").lower() == "true"
if _ENABLE_EMAIL_POLLING and (_now - st.session_state.last_auto_fetch) >= _FETCH_INTERVAL:
    if EMAIL_USER and EMAIL_PASSWORD and DATABASE_URL:
        try:
            _auto_result = fetch_emails_from_gmail(
                include_read=_INCLUDE_READ or _FETCH_ALL,
                max_emails=5,
            )
...
if st.session_state.page == "fetch":
    ...
    with c1:
        include_read = st.checkbox("Include already-read emails", value=True)
    with c2:
...
# ══════════════════════════════════════════════════════════════════
#  TAB: QUIZ (integrated) — calls backend /quiz endpoints
# ══════════════════════════════════════════════════════════════════
elif st.session_state.tab == "quiz":
    st.markdown(top_bar("Finance Quiz"), unsafe_allow_html=True)

    BACKEND_URL = _secret("BACKEND_API_URL", _secret("BACKEND_URL", "http://127.0.0.1:8001"))
    ...
    if "quiz_topic" not in st.session_state:
        st.session_state.quiz_topic = "Finance"
    ...
    with st.sidebar:
        ...
        st.session_state.quiz_num_questions = st.slider("Questions", 1, 10, st.session_state.quiz_num_questions)
        if st.button("Generate Quiz", key="generate_quiz", use_container_width=True):
            import requests
            try:
                resp = requests.get(
                    f"{BACKEND_URL}/quiz/generate",
                    params={"topic": st.session_state.quiz_topic, "num_questions": st.session_state.quiz_num_questions},
                    timeout=120,
                )
                resp.raise_for_status()
                payload = resp.json()
                st.session_state.quiz_data = payload.get("data", payload)
                st.session_state.current_q = 0
                st.session_state.score = 0
                st.session_state.complete = False
    ...
    if not questions:
        st.error("No questions returned by backend.")
        st.json(quiz_data)
        st.stop()

    if st.session_state.complete:
        st.success(f"Quiz complete: {st.session_state.score}/{len(questions)}")
    ...
    display_options = [opt.get("text", str(opt)) if isinstance(opt, dict) else str(opt) for opt in options]
    selected = st.radio("Select an option", display_options, key=f"quiz_{q_idx}")

    if st.button("Submit Answer", key=f"submit_{q_idx}", use_container_width=True):
        import requests
        try:
            resp = requests.post(
                f"{BACKEND_URL}/quiz/score",
                json={
                    "question_id": q.get("id", q_idx + 1),
            ...
            if result.get("is_correct"):
                ...
                st.error(f"Wrong. Correct answer: {result.get('correct_answer', 'N/A')}")
            ...
            st.error(f"Quiz scoring failed: {e}")
