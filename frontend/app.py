...
#  AUTO-REFRESH
# ═══════════════════════════════════════════════════════
_AUTO_REFRESH_MS = int(os.environ.get("UI_REFRESH_INTERVAL_MS", "30000"))
st_autorefresh(interval=_AUTO_REFRESH_MS, limit=None, key="auto_refresh")
...
def _api_request(method: str, path: str, **kwargs):
    """Centralized request helper. Returns parsed JSON or None on error."""
...
def _fetch_tickets(status: str = "All"):
    ...
    res = _api_request("get", "/tickets", params=params, timeout=10)
    return res or []


def _api_approve(tid: str):
    res = _api_request("post", f"/approve_ticket/{tid}", timeout=30)
    return res


def _api_close(tid: str):
    res = _api_request("patch", f"/tickets/{tid}/reject", timeout=10)
    return bool(res is not None)


def _api_mark_read(tid: str):
    res = _api_request("put", f"/tickets/{tid}/read", timeout=5)
    return bool(res is not None)
...
# ═══════════════════════════════════════════════════════
#  AUTO-FETCH (backend ingestion)
# ═══════════════════════════════════════════════════════
_EMAIL_POLL_INTERVAL = int(os.environ.get("EMAIL_POLL_INTERVAL", "300"))
_ENABLE_EMAIL_POLLING = os.environ.get("ENABLE_EMAIL_POLLING", "true").lower() == "true"
_INCLUDE_READ = os.environ.get("EMAIL_INCLUDE_READ", "false").lower() == "true"
_FETCH_ALL = os.environ.get("EMAIL_FETCH_ALL", "false").lower() == "true"

_now = _time.time()
if _ENABLE_EMAIL_POLLING and (_now - st.session_state.last_auto_fetch) >= _EMAIL_POLL_INTERVAL:
    # Trigger backend email ingestion on a cadence (default: 5 min)
    _api_fetch_emails(include_read=_INCLUDE_READ or _FETCH_ALL, max_emails=5)
...
def _api_quiz_generate(topic: str = "Finance", num_questions: int = 5):
    """Generate a quiz from backend quiz service using centralized helper."""
    res = _api_request(
        "get",
        "/quiz/generate",
...
def _api_quiz_score(question_id: int, selected_option: str, quiz_data: dict):
    """Score an answer using backend quiz service via centralized helper."""
    res = _api_request(
        "post",
        "/quiz/score",
        json={
...
# ──────────────────────────────────────────────────────
    ...
    # Quiz health + model reload
    quiz_health = _api_request("get", "/quiz/health", timeout=5)
    if quiz_health is None:
        st.warning("Quiz service unreachable — backend may be offline.")
    else:
        status_text = "LLM available" if quiz_health.get("llm_available") else "Running in mock mode"
        st.caption(f"Quiz: {status_text} — model={quiz_health.get('model')}")
        if not quiz_health.get("llm_available"):
            if st.button("Restart quiz model", use_container_width=True):
                with st.spinner("Reloading quiz engine..."):
                    reload_res = _api_request("post", "/quiz/reload", timeout=30)
                if reload_res and reload_res.get("llm_available"):
                    st.success("Quiz model reloaded and available.")
                else:
                    st.info("Reloaded but model still unavailable — using mock quiz.")
