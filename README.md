...
- **Auto-fetch emails** from Gmail via IMAP on a configurable interval while the app is active
...
### Keep-Alive Notes (Streamlit Cloud)

Streamlit Community Cloud may sleep apps after inactivity. Auto-refresh and email polling
only run while someone has the app open. If you need always-on fetching, use an external
keep-alive ping or run the backend worker on a VM.

This repo includes a GitHub Actions keep-alive workflow at:
.github/workflows/streamlit-keepalive.yml
Set the repo secret `STREAMLIT_APP_URL` to your app URL.
