...
@quiz_router.post("/reload")
async def reload_quiz() -> Dict:
    """Reinitialize the quiz engine (useful if the LLM client needs to be reloaded)."""
    global quiz_engine
    try:
        quiz_engine = QuizEngine()
        return {
        ...
        logger.error(f"Error reloading quiz engine: {e}")
        raise HTTPException(status_code=500, detail=str(e))
