...
import re, smtplib, logging, asyncio
...
async def lifespan(app: FastAPI):
    ...
    with engine.begin() as conn:
        ...
        if "is_read" not in columns:
            ...
            print("  [OK] Added is_read column to tickets table.")
        ...
        if "is_ai_draft_edited" not in columns:
            ...
            print("  [OK] Added is_ai_draft_edited column to tickets table.")

    print("[OK] Database tables are ready.")

    # ── Background email polling (Railway keeps the process alive) ──
    EMAIL_POLL_INTERVAL = int(os.getenv("EMAIL_POLL_INTERVAL", "300"))  # seconds (5 min default)
    ...
    async def _background_email_poller():
        ...
        logger.info(f"[EMAIL] Background email poller started (interval={EMAIL_POLL_INTERVAL}s)")
        while True:
            try:
                ...
                if resp.ok:
                    ...
                    if fetched > 0:
                        logger.info(f"[EMAIL] Background poller: {fetched} new email(s) processed")
                    else:
                        logger.debug("[EMAIL] No new emails")
                else:
                    logger.warning(f"[EMAIL] Background poller: API returned {resp.status_code}")
            except Exception as e:
                logger.warning(f"[EMAIL] Background poller error (will retry): {e}")
            await asyncio.sleep(EMAIL_POLL_INTERVAL)

    poll_task = None
    if ENABLE_EMAIL_POLLING:
        poll_task = asyncio.create_task(_background_email_poller())
        logger.info("[EMAIL] Email polling enabled (set ENABLE_EMAIL_POLLING=false to disable)")
    else:
        logger.info("[EMAIL] Email polling disabled")
...
def fetch_emails_endpoint(
    db: Session = Depends(get_db),
    include_read: bool = Query(True, description="Also fetch already-read emails"),
    ...
    try:
        ...
        # If explicitly asking to include read mails or fetch_all enabled => search recent mailboxes
        if include_read or fetch_all:
            ...
            # Default: only unread items from INBOX; optionally include Sent/All Mail (recent) when requested
